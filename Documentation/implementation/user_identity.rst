...
``CONFIG_FS_PERMISSION``
  ...
  See :ref:`fs-permission-unified-iface` for the VFS helpers, mount-crossing
...
via ``inode_checksearchpath()``.  Open-time checks on the final node use
``inode_checkopenperm()`` (or ``inode_permission()`` for named IPC
objects).  Full details, including mounts under private pseudoFS parents,
are in :ref:`fs-permission-unified-iface`.
